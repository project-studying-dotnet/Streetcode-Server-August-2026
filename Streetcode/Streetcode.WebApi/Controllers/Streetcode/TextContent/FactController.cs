...
public class FactController : BaseApiController
{
    ...
    {
        var command = new CreateFactCommand(fact);

        return HandleResult(await Mediator.Send(command));
    }

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] FactUpdateCreateDto fact)
    {
        var command = new UpdateFactCommand(id, fact);
